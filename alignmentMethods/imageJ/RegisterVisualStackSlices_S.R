...
#source_dir <- "/Users/vagm_110901/Documents/Universitat/ConesaLab/Lab/05.ImageJ/ImagenesAlinearFiji/mirrorXY"
source_dir <- "/Users/vagm_110901/Documents/Universitat/ConesaLab/Lab/05.ImageJ/ImagenesAlinearFiji/paciente19"
#target_dir <- "/Users/vagm_110901/Documents/Universitat/ConesaLab/Lab/05.ImageJ/ImagenesAlinearFiji/mirrorXYaligned"
target_dir <- "/Users/vagm_110901/Documents/Universitat/ConesaLab/Lab/05.ImageJ/ImagenesAlinearFiji/paciente19aligned_scale"
#ref_name <- "original"
...
for (xml in files_xml) {
  ...
  if (!startsWith(xml, ref_name)) {
    ...
    # Extract rotation, translation and scale parameters based on transform class type
    for (line in lines) {
      classN <- which(startsWith(line, " class="))
      if (endsWith(line[[classN + 1]], "transform.SimilarityModel2D")) { 
        dataN <- which(startsWith(line, " data="))
        transformsS <- line[[dataN + 1]]
        transformsS <- strsplit(transformsS, " ")
      }
...
for (i in 2:length(pacientes.semla@tools$Staffli@rasterlists$raw)) {
  ...
  if ( valores$dx != 0 || valores$dy != 0 ) {
    transforms_trans <- generate_rigid_transform(sampleID = i, 
                                           tr_x = valores$dx, #round(valores[[2]], digits = 2), 
                                           tr_y = valores$dy) #round(valores[[3]], digits = 2),
...
png(paste0(saveDir, "/results/patient19_merge_imageJ_S.png"))
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_region.png"), plot = palign)
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_genesmt.png"), plot = pmt)

###############################################################################

saveRDS(pacientes.seurat, paste0(saveDir, "/results/patient19_merge_imageJ_S.rds"))
#pacientes.seurat <- readRDS(paste0(saveDir, "/results/patient19_merge_imageJ_S.rds"))
#pacientes.semla <- pacientes.seurat
saveRDS(Evaluation, paste0(saveDir, "/results/patient19_EVALUATION_merge_imageJ_S.rds"))
...
for (i in 2:length(pacientes.seurat.split.orig)) {
  ...
  savename <- paste0("/results/patient19_merge_imageJ_S_list_im", i, ".rds")
  saveRDS(listaObjDeconv, paste0(saveDir, savename))
}

############################################################################### Statistical part
Evaluation <- readRDS(paste0(saveDir, "/results/patient19_EVALUATION_merge_imageJ_S.rds"))
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_evalMSE.png"), plot = pMSE)
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_evalMSEgray.png"), plot = pMSEgray)
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_evalSSIM.png"), plot = pSSIM)
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_evalEucl.png"), plot = pEucl)
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_eval1.png"), plot = peval1)
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_eval2.png"), plot = peval2)
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_eval3.png"), plot = peval3)
...
ggsave(paste0(saveDir, "/results/patient19_merge_imageJ_S_eval4.png"), plot = peval4)
