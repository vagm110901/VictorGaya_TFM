...
library(tibble)
library(IMIFA)
source("/Users/vagm_110901/Documents/Universitat/ConesaLab/Lab/04.ImageAlingment/function_selectCoord_procrustes.R")
...
#pacientes.semla <- UpdateSeuratForSemla(pacienteInv.integratedPCA50)
...
for (i in 2:length(pacientes.semla@tools$Staffli@rasterlists$raw)) {
  ...
  for (j in seq_along(coordenadas2)) {coordenadas2[[j]] <- round(coordenadas2[[j]])
  }
  ...
  proc <- Procrustes(X = matProb, Xstar = matrixCoord1, translate = TRUE, sumsq = TRUE)
  coseno <- proc$R[1,1]
  seno <- proc$R[2,1]
  dx <- proc$t[1]
  dy <- proc$t[2]
  mirrorx <- FALSE
  mirrory <- FALSE
  solucionOrig <- c(coseno, seno, dx, dy, mirrorx, mirrory)
  names(solucionOrig) <- c('coseno', 'seno', 'dx', 'dy', 'mirrorx', 'mirrory')
  ...
  mirrorx <- TRUE
  mirrory <- FALSE
  solucionMirrorX <- c(coseno, seno, dx, dy, mirrorx, mirrory)
  names(solucionMirrorX) <- c('coseno', 'seno', 'dx', 'dy', 'mirrorx', 'mirrory')
  ...
  mirrorx <- FALSE
  mirrory <- TRUE
  solucionMirrorY <- c(coseno, seno, dx, dy, mirrorx, mirrory)
  names(solucionMirrorY) <- c('coseno', 'seno', 'dx', 'dy', 'mirrorx', 'mirrory')
  ...
  mirrorx <- TRUE
  mirrory <- TRUE
  solucionMirrorXY <- c(coseno, seno, dx, dy, mirrorx, mirrory)
  names(solucionMirrorXY) <- c('coseno', 'seno', 'dx', 'dy', 'mirrorx', 'mirrory')
...
png(paste0(saveDir, "/results/patient19_merge_procrustes.png"))
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_region.png"), plot = palign)
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_genesmt.png"), plot = pmt)

###############################################################################


saveRDS(pacientes.seurat, paste0(saveDir, "/results/patient19_merge_procrustes.rds"))
#pacientes.seurat <- readRDS(paste0(saveDir, "/results/patient19_merge_procrustes.rds"))
#pacientes.semla <- pacientes.seurat
saveRDS(Evaluation, paste0(saveDir, "/results/patient19_EVALUATION_merge_procrustes.rds"))
...
for (i in 2:length(pacientes.seurat.split.orig)) {
  ...
  savename <- paste0("/results/patient19_merge_procrustes_list_im", i, ".rds")
  saveRDS(listaObjDeconv, paste0(saveDir, savename))
}

############################################################################### Statistical part
Evaluation <- readRDS(paste0(saveDir, "/results/patient19_EVALUATION_merge_procrustes.rds"))
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_evalMSE.png"), plot = pMSE)
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_evalMSEgray.png"), plot = pMSEgray)
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_evalSSIM.png"), plot = pSSIM)
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_evalEucl.png"), plot = pEucl)
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_eval1.png"), plot = peval1)
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_eval2.png"), plot = peval2)
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_eval3.png"), plot = peval3)
...
ggsave(paste0(saveDir, "/results/patient19_merge_procrustes_eval4.png"), plot = peval4)
