...
############################################################################### Invert X Axis (x, y) -> (-x, y)

```{r}
#| label: function_inversionX
source("/Users/vagm_110901/Documents/Universitat/ConesaLab/Lab/02.Seurat/FlipFunction.R") # Source the function for inverting the X axis
```

```{r}
#| label: sample1_2_inversionX

plot1 <- SpatialDimPlot(sample1, alpha = 0) & theme(legend.position = "none") & ggtitle("Sample 1") # Create spatial plot for original sample

sample2 <- inversionEjeX(sample1) # Invert the X axis of sample1
...
sample3 <- inversionEjeX(sample2) # Invert the X axis of sample2 back to recover original orientation
...
jpeg(filename = paste0(output_directory, "pacientes_spatial_pngX.jpg"), height = 700, width = 1200, quality = 50) # Save the combined plot as a JPEG image
...
saveRDS(merged_patients, # Save the merged Seurat object as an RDS file
        file = paste0(output_directory, "/Pacientes_mergeX.rds"))
merged_patients <- readRDS(file = paste0(output_directory, "/Pacientes_mergeX.rds"))
```
...
jpeg(filename = paste0(saveDir, "pacientes_spatial_pngX_spots.jpg"), height = 700, width = 1200, quality = 50)
SpatialPlot(object = merged_patients, 
            features = "percent.butterfly", alpha = 1, crop = FALSE, ncol = 3) & 
